...
public class MetalinkDownloader {
    ...
    public MetalinkDownloader(String login, String password, String tomtomVersion) {
        this.login = login;
    ...
    public Metalink download() throws IOException {
        ...
        File outputFile = new File("./Europe." + now().toString(forPattern("ddMMyyyy-HHmm")) + ".metalink");
        log.info("Fetching metalink in path={}", outputFile.getAbsolutePath());
        try (InputStream content = client.execute(metalinkRequest).getEntity().getContent(); FileOutputStream fos = new FileOutputStream(outputFile)) {
