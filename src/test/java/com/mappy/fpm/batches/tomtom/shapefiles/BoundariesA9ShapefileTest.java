...
public class BoundariesA9ShapefileTest extends AbstractTest {
    ...
    public static void setup() {
        ...
        when(nameProvider.getAlternateNames(10560000419571L)) //
                .thenReturn(of("name", "Ranst", "name:nl", "RanstCNL", "name:fr", "RanstCFR"));
        when(nameProvider.getAlternateNames(10560000309610L)) //
                .thenReturn(of("name", "Broechem", "name:nl", "BroechemCNL", "name:fr", "BroechemCFR"));
        when(nameProvider.getAlternateNames(10560000712819L)) //
                .thenReturn(of("name", "Emblem", "name:nl", "EmblemCNL", "name:fr", "EmblemCFR"));
        when(nameProvider.getAlternateNames(10560000571768L)) //
        ...
        when(townTagger.getCityCentroid(10560000419571L)).thenReturn(new Centroid(10560000419571L, "Ranst", "123", 8, 1, 7, point));

        Point point2 = new Point(new PackedCoordinateSequence.Double(new double[]{4.601984, 51.181340}, 2), factory);
        when(townTagger.getCityCentroid(10560000309610L)).thenReturn(new Centroid(10560000309610L, "Broechem", "456", 8, 1, 8, point2));

        Point point3 = new Point(new PackedCoordinateSequence.Double(new double[]{4.606374, 51.162370}, 2), factory);
        when(townTagger.getCityCentroid(10560000712819L)).thenReturn(new Centroid(10560000712819L, "Emblem", null, 8, 1, 8, point3));

        Point point4 = new Point(new PackedCoordinateSequence.Double(new double[]{4.596975, 51.210989}, 2), factory);
        when(townTagger.getCityCentroid(10560000571768L)).thenReturn(new Centroid(10560000571768L, "Oelegem", "1011", 8, 1, 8, point4));

        BoundariesA9Shapefile shapefile = new BoundariesA9Shapefile(tomtomFolder, townTagger, nameProvider, osmLevelGenerator);
